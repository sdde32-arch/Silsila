...
# Replace the mangled buttons
buttons_fix = """          <div className="flex items-center gap-1.5">
            <button
...
content = re.sub(r'<div className="flex items-center gap-1\.5">.*?</div>\s*</div>\s*\{isLoading \?', buttons_fix + '\n\n        {isLoading ?', content, flags=re.DOTALL)

# Delete keyRoots entirely
content = re.sub(r'\{aiInsights\?\.keyRoots.*?</div>\s*\)\}\s*', '', content, flags=re.DOTALL)
